...
import com.hontoka.truckboss.service.ScheduleService;
...
/**
 * REST controller for managing {@link com.hontoka.truckboss.domain.Schedule}.
 */
@RestController
@RequestMapping("/api")
public class ScheduleResource {
    ...
    @PostMapping("/start_journey")
    public ResponseEntity<JourneyDTO> startJourney(@RequestBody ScheduleDTO scheduleDTO) throws URISyntaxException {
        log.debug("REST request to save Schedule : {}", scheduleDTO);
        if (scheduleDTO.getId() == null) {
            throw new BadRequestAlertException("A new journey must belongs to an existing schedule", ENTITY_NAME, "idnull");
        }
        JourneyDTO result = scheduleService.startJourney(scheduleDTO.getId());
    ...
    public ResponseEntity<List<ScheduleDTO>> getAllSchedules(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
