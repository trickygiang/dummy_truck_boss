...
class TruckBossKafkaResourceIT {
    ...
    void setup() {
        ...
        TruckBossKafkaResource kafkaResource = new TruckBossKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/truck-boss-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/truck-boss-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
